<template>
  <div ref="triggerRef" class="trigger-container" v-bind="$attrs">
    <slot></slot>
  </div>
  ...
  <Teleport to="body" v-if="isMounted">
    <Transition :name="getTransitionName">
      ...
      >
        ...
        <div class="drop-panel__content" ref="contentRef">
          <span>{{ title }}</span>
        </div>
      </div>
    </Transition>
...
const checkOverflow = (rect) => {
  ...
  return {
    ...
    any: rect.top < viewport.top || rect.right > viewport.right || 
         rect.bottom > viewport.bottom || rect.left < viewport.left
  };
};
...
const calcPanelPosition = async() => {
  if (!triggerRef.value || !panelRef.value || !contentRef.value || !showPanel.value) return;
...
const handleTriggerClick = () => {
  ...
  nextTick(() => {
    ...
    if (showPanel.value) calcPanelPosition(); // 调用优化后的计算函数
  });
};
// 事件绑定
...
.slide-up-enter-from { opacity: 0; transform: translateY(8px) scaleY(0.95); }
.slide-up-enter-to, .slide-up-leave-from { opacity: 1; transform: translateY(0) scaleY(1); }
.slide-up-leave-to { opacity: 0; transform: translateY(8px) scaleY(0.95); }
.slide-down-enter-from { opacity: 0; transform: translateY(-8px) scaleY(0.95); }
.slide-down-enter-to, .slide-down-leave-from { opacity: 1; transform: translateY(0) scaleY(1); }
.slide-down-leave-to { opacity: 0; transform: translateY(-8px) scaleY(0.95); }
.slide-left-enter-from { opacity: 0; transform: translateX(8px) scaleX(0.95); }
.slide-left-enter-to, .slide-left-leave-from { opacity: 1; transform: translateX(0) scaleX(1); }
.slide-left-leave-to { opacity: 0; transform: translateX(8px) scaleX(0.95); }
.slide-right-enter-from { opacity: 0; transform: translateX(-8px) scaleX(0.95); }
.slide-right-enter-to, .slide-right-leave-from { opacity: 1; transform: translateX(0) scaleX(1); }
.slide-right-leave-to { opacity: 0; transform: translateX(-8px) scaleX(0.95); }
</style>
